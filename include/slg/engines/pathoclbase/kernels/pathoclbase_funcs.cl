...
OPENCL_FORCE_INLINE void RespirReservoir_Update(const __global GPUTaskConfiguration* restrict taskConfig, __global GPUTaskState* restrict taskState, 
	...
	// unnormalized path contribution
	float3 pathContribution = SampleResult_GetUnscaledSpectrum(&taskConfig->film, newSample) / VLOAD3F(taskState->throughput.c) / VLOAD3F(taskState->lastWeight.c);

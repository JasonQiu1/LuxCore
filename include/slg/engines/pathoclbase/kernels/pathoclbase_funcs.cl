...
OPENCL_FORCE_INLINE bool Respir_UpdateNextNeighborGid(GPUTaskState* taskState, 
	...
	int searchX = sampleResult->pixelX + (int) copysign(floorf(Rnd_FloatValue(seed) * spatialRadius) + 1), Rnd_FloatValue(seed) - 0.5f);
	int searchY = sampleResult->pixelY + (int) copysign(floorf(Rnd_FloatValue(seed) * spatialRadius) + 1), Rnd_FloatValue(seed) - 0.5f);

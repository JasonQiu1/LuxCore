...
OPENCL_FORCE_INLINE void GenerateEyePath(
	...
	Radiance_Clear(taskState->initialPathReservoir.selectedSample.normalizedRadiance);
	Radiance_Clear(taskState->initialPathReservoir.selectedSample.reconnectionVertex.normalizedRadiance);
#endif
}
...
OPENCL_FORCE_INLINE bool Respir_UpdateNextNeighborGid(GPUTaskState* taskState, 
	...
	int searchX = sampleResult->pixelX + (int) copysign(floorf(Rnd_FloatValue(seed) * spatialRadius) + 1.0f, Rnd_FloatValue(seed) - 0.5f);
	int searchY = sampleResult->pixelY + (int) copysign(floorf(Rnd_FloatValue(seed) * spatialRadius) + 1.0f, Rnd_FloatValue(seed) - 0.5f);

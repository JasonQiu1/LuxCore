...
OPENCL_FORCE_INLINE void GenerateEyePath(
	...
	taskState->initialPathReservoir.sumConfidence = 0.0f;
	taskState->totalThroughput = 1.0f;
...
OPENCL_FORCE_INLINE void DirectHitInfiniteLight(__constant const Film* restrict film,
	...
	for (uint i = 0; i < envLightCount; ++i) {
		...
		if (!Spectrum_IsBlack(envRadiance)) {
			...
			SampleResultReservoir_Add(&taskState->initialPathReservoir, weight, &taskState->seedReservoirSampling, sampleResult);
		}
	}
}
